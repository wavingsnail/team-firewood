...
public static class MathsUtils {
    ...
    public static void Test() {
        ...
        v = v.GetWithMagnitude(1000);
        DebugUtils.Assert(Mathf.Approximately(v.x, 1000), "v.x = " + v.x);
        DebugUtils.Assert(Mathf.Approximately(v.y, 0), "v.x = " + v.y);

        v.x = 2;
        v.y = 0;
        var v2 = v.GetWithMagnitude(1000);
        DebugUtils.Assert(Mathf.Approximately(v.x, 2), "v.x = " + v.x);
        DebugUtils.Assert(Mathf.Approximately(v2.x, 1000), "v2.x = " + v2.x);
        DebugUtils.Assert(Mathf.Approximately(v2.y, 0), "v2.x = " + v2.y);

        DebugUtils.Log("MathsUtils - OK!");
    }
#endif
}
}
